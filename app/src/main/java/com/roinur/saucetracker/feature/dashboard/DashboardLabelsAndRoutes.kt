...
internal fun tagSortArrow(vm: com.roinur.saucetracker.feature.dashboard.DashboardViewModel, field: TagSortField): String {
    if (vm.tagSortField != field) return ""
    return if (vm.tagSortDirection == SortDirection.DESC) " ▼" else " ▲"
}

internal fun blockedTagSortArrow(vm: com.roinur.saucetracker.feature.dashboard.DashboardViewModel, field: TagSortField): String {
    if (vm.blockedTagSortField != field) return ""
    return if (vm.blockedTagSortDirection == SortDirection.DESC) " ▼" else " ▲"
}

internal fun creatorSortArrow(vm: com.roinur.saucetracker.feature.dashboard.DashboardViewModel, field: CreatorSortField): String {
    if (vm.creatorSortField != field) return ""
    return if (vm.creatorSortDirection == SortDirection.DESC) " ▼" else " ▲"
}

internal fun entrySortArrow(vm: com.roinur.saucetracker.feature.dashboard.DashboardViewModel, field: EntrySortField): String {
...
internal fun dashboardDiscoveryPageLabel(page: DashboardDiscoveryPage): String = when (page) {
    DashboardDiscoveryPage.SUGGESTED -> "Suggested entries"
    DashboardDiscoveryPage.RANDOM -> "Random entries"
    DashboardDiscoveryPage.SAUCE_FINDER -> "Sauce Finder"
}

internal fun dashboardInsightPageLabel(page: DashboardInsightPage): String = when (page) {
    DashboardInsightPage.SUBSCRIPTIONS -> "Subscriptions"
    DashboardInsightPage.HEATMAP -> "Heatmap"
